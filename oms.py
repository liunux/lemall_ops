...
def app_list(usertype,nickname,badge):
    ...
    if mohu:
        word = '%'+word+'%'
    if word:
        applicationsql = "select a.*,count(b.app_id),c.out_ip from ops_application a,ops_instance b,ops_machine c where a.app_id=b.app_id and b.ip = c.in_ip and b.ip !='' and location " \
        ...
        applicationsql = "select a.*,count(b.app_id) from ops_application a,ops_instance b where a.app_id=b.app_id and b.ip != '' and  location " \
